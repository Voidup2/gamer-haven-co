...
class MarketplaceTransactionServiceTest {
    ...
    void findByIdAllowsBuyer() {
        UUID id = UUID.randomUUID();
        authenticateAs("buyer", 2L, buyer);
        MarketplaceTransactionResponse response = givenTransaction(id, 1L, 2L, MarketplaceTransaction.Status.PENDING);
    ...
    void findMineAsBuyerUsesBuyerRepositoryQuery() {
        ...
        Page<MarketplaceTransaction> page = new PageImpl<>(List.of());
    ...
    void findMineAsSellerUsesStatusQuery() {
        ...
        PageRequest pageable = PageRequest.of(0, 20);
        Page<MarketplaceTransaction> page = new PageImpl<>(List.of());
        when(transactionRepository.findBySellerIdAndStatusOrderByCreatedAtDesc(1L, MarketplaceTransaction.Status.COMPLETED, pageable)).thenReturn(page);
    ...
    void findAllAllowsAdmin() {
        ...
        org.mockito.MockedStatic<UserRoleHelper> unused = null;
    }
