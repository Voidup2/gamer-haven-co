...
public class ChatService {
    ...
    @Transactional
    public ChatDtos.RoomResponse global() {
        return createOrJoin(ChatRoomType.GLOBAL, "global", "Global Chat", null, null);
    }

    @Transactional
    public ChatDtos.RoomResponse trade() {
        return createOrJoin(ChatRoomType.TRADE, "trade", "Trade Chat", null, null);
    }
    ...
    public ChatDtos.RoomResponse group(UUID groupId) {
        ...
        if (!groupMemberRepository.existsByGroupIdAndUserId(groupId, user.getId())) {
            throw new AccessDeniedException("You must be a group member to enter its chat");
        }
    ...
    public ChatDtos.RoomResponse direct(Long otherUserId) {
        ...
        if (user.getId().equals(otherUserId)) {
            throw new ConflictException("You cannot start a direct chat with yourself");
        }
        User other = userRepository.findById(otherUserId)
                .orElseThrow(() -> new ResourceNotFoundException("User not found: " + otherUserId));
        long first = Math.min(user.getId(), other.getId());
        long second = Math.max(user.getId(), other.getId());
    ...
    public ChatDtos.RoomResponse join(UUID roomId) {
        ...
        if (room.getRoomType() == ChatRoomType.GROUP && room.getGroup() != null
                && !groupMemberRepository.existsByGroupIdAndUserId(room.getGroup().getId(), user.getId())) {
            throw new AccessDeniedException("You must be a group member to enter its chat");
        }
        if (room.getRoomType() == ChatRoomType.DIRECT && !memberRepository.existsByRoomIdAndUserId(roomId, user.getId())) {
    ...
    public void leave(UUID roomId) {
        User user = currentUser();
        ChatRoom room = room(roomId);
        if (room.getRoomType() == ChatRoomType.GLOBAL || room.getRoomType() == ChatRoomType.TRADE
                || room.getRoomType() == ChatRoomType.DIRECT) {
    ...
    @Transactional
    public Page<ChatDtos.RoomResponse> myRooms(Pageable pageable) {
        User user = currentUser();
        return roomRepository.findRoomsForUser(user.getId(), pageable).map(this::toRoomResponse);
    }

    @Transactional
    public Page<ChatDtos.MessageResponse> messages(UUID roomId, Pageable pageable) {
        User user = currentUser();
        ChatRoom room = room(roomId);
        ensureCanAccess(room, user);
        return messageRepository.findByRoomIdAndDeletedAtIsNull(roomId, pageable).map(this::toMessageResponse);
    }

    @Transactional
    public ChatDtos.MessageResponse send(UUID roomId, ChatDtos.SendMessageRequest request) {
        User user = currentUser();
        ChatRoom room = room(roomId);
        ensureCanAccess(room, user);
    ...
    @Transactional
    public ChatDtos.MessageResponse edit(UUID messageId, ChatDtos.EditMessageRequest request) {
        User user = currentUser();
    ...
    @Transactional
    public void markRead(UUID roomId) {
        User user = currentUser();
        ChatRoom room = room(roomId);
        ensureCanAccess(room, user);
        ChatRoomMember member = memberRepository.findByRoomIdAndUserId(roomId, user.getId())
                .orElseThrow(() -> new AccessDeniedException("You are not a member of this chat"));
        member.setLastReadAt(OffsetDateTime.now());
        memberRepository.save(member);
    }

    private ChatDtos.RoomResponse createOrJoin(ChatRoomType type, String key, String name, Game game, GameGroup group) {
        User user = currentUser();
        ChatRoom room = roomRepository.findByRoomKey(key).orElseGet(() ->
                roomRepository.save(new ChatRoom(UUID.randomUUID(), type, key, name, game, group, user)));
        ensureMember(room, user);
        return toRoomResponse(room);
    }
    ...
    private ChatDtos.RoomResponse toRoomResponse(ChatRoom room) {
        ...
        return new ChatDtos.RoomResponse(room.getId(), room.getRoomType(), room.getRoomKey(), room.getName(), gameId,
                groupId, memberRepository.countByRoomId(room.getId()), room.getCreatedAt());
    }

    private ChatDtos.MessageResponse toMessageResponse(ChatMessage message) {
        User sender = message.getSender();
        return new ChatDtos.MessageResponse(message.getId(), message.getRoom().getId(), sender.getId(),
                sender.getUsername(), sender.getDisplayName(), message.getContent(), message.getCreatedAt(), message.getEditedAt());
    }
