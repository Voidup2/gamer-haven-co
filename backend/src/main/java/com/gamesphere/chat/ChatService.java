...
public class ChatService {
    ...
    @Transactional public ChatDtos.RoomResponse trade() { return toRoomResponse(createOrJoin(ChatRoomType.TRADE, "trade", "Trade Chat", null, null)); }

    @Transactional public ChatDtos.RoomResponse game(String gameId) {
    ...
    @Transactional public ChatDtos.RoomResponse group(UUID groupId) {
        User user = currentUser(); GameGroup group = groupRepository.findById(groupId).orElseThrow(() -> new ResourceNotFoundException("Group not found: " + groupId));
    ...
    @Transactional public ChatDtos.RoomResponse direct(Long otherUserId) {
        ...
        ChatRoom room = roomRepository.findByRoomKey(key).orElseGet(() -> {
            ...
            memberRepository.save(new ChatRoomMember(created, user)); memberRepository.save(new ChatRoomMember(created, other)); return created;
        });
        ensureMember(room, user); return toRoomResponse(room);
    }

    @Transactional public ChatDtos.RoomResponse join(UUID roomId) {
        User user = currentUser(); ChatRoom room = room(roomId); ensureCanAccess(room, user); realtimePublisher.roomMemberChanged(ChatRealtimeEventType.ROOM_JOINED, roomId, user.getId(), user.getUsername()); return toRoomResponse(room);
    }

    @Transactional public void leave(UUID roomId) {
        ...
        if (memberRepository.findByRoomIdAndUserId(roomId, user.getId()).isPresent()) {
            memberRepository.deleteByRoomIdAndUserId(roomId, user.getId());
            realtimePublisher.roomMemberChanged(ChatRealtimeEventType.ROOM_LEFT, roomId, user.getId(), user.getUsername());
        }
    }

    @Transactional public Page<ChatDtos.RoomResponse> myRooms(Pageable pageable) { User user = currentUser(); return roomRepository.findRoomsForUser(user.getId(), pageable).map(this::toRoomResponse); }
    ...
    @Transactional public void markRead(UUID roomId) {
        ...
        member.setLastReadAt(OffsetDateTime.now()); memberRepository.save(member); realtimePublisher.read(roomId, user.getId(), user.getUsername());
    }
