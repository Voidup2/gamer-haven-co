...
public class ForumService {
    ...
    public ForumService(ForumTopicRepository topicRepository, ForumPostRepository postRepository, UserRepository userRepository, UserBlockService blockService) {
    ...
    @Transactional public ForumDtos.PostResponse createPost(UUID topicId, ForumDtos.CreatePostRequest request) {
        ...
        if (blockService.isEitherBlocked(user.getId(), topic.getAuthor().getId())) throw new org.springframework.security.access.AccessDeniedException("You cannot interact with this user");
        ForumPost post = new ForumPost(topic, user, clean(request.content())); return toPostResponse(postRepository.save(post));
    }
    ...
    private User currentUser() { Authentication authentication = SecurityContextHolder.getContext().getAuthentication(); if (authentication == null || !authentication.isAuthenticated()) throw new IllegalStateException("Authentication required"); return userRepository.findByUsername(authentication.getName()).orElseThrow(() -> new ResourceNotFoundException("Current user not found")); }
    private void assertOwnerOrAdmin(User owner, User current) { if (!owner.getId().equals(current.getId()) && !isAdmin(current)) throw new org.springframework.security.access.AccessDeniedException("Not allowed"); }
    private void assertAdmin(User user) { if (!isAdmin(user)) throw new org.springframework.security.access.AccessDeniedException("Admin access required"); }
