...
public class GroupService {
    ...
    public GroupResponse create(GroupRequest request) {
        ...
        if (groupRepository.existsByOwnerIdAndName(user.getId(), request.name())) {
            throw new ConflictException("You already have a group with this name");
        }
        GameGroup group = groupRepository.save(new GameGroup(user, request.name(), request.description(),
                request.imageUrl(), request.publicGroup()));
    ...
    public Page<GroupResponse> mine(Pageable pageable) {
        User user = currentUser();
        return groupRepository.findByOwnerIdOrderByCreatedAtDesc(user.getId(), pageable)
                .map(group -> toResponse(group, user.getId()));
    }
    ...
    public GroupResponse findById(UUID id) {
        ...
        User user = currentUserOrNull();
        if (!group.isPublicGroup() && (user == null || !isMember(group, user.getId()))) {
            throw new AccessDeniedException("You must be a group member to view this group");
        }
    ...
    public GroupResponse update(UUID id, GroupRequest request) {
        ...
        if (groupRepository.existsByOwnerIdAndNameAndIdNot(user.getId(), request.name(), id)) {
            throw new ConflictException("You already have a group with this name");
        }
    ...
    public GroupResponse join(UUID id) {
        ...
        if (!group.isPublicGroup()) {
            throw new AccessDeniedException("This group is private");
        }
    ...
    public void leave(UUID id) {
        ...
        if (group.getOwner().getId().equals(user.getId())) {
            throw new ConflictException("The group owner cannot leave the group");
        }
        if (!memberRepository.existsByGroupIdAndUserId(id, user.getId())) {
    ...
    public List<GroupMemberResponse> members(UUID id) {
        ...
        if (!group.isPublicGroup()) {
            User user = currentUser();
            if (!isMember(group, user.getId())) {
                throw new AccessDeniedException("You must be a group member to view members");
            }
        }
        return memberRepository.findByGroupIdOrderByJoinedAtAsc(id).stream()
    ...
    public boolean isMember(UUID id) {
        User user = currentUser();
        return groupRepository.existsById(id) && memberRepository.existsByGroupIdAndUserId(id, user.getId());
    }

    private GameGroup ownedGroup(UUID id, Long userId) {
        return groupRepository.findByIdAndOwnerId(id, userId)
                .orElseThrow(() -> new AccessDeniedException("Only the group owner can modify this group"));
    }

    private boolean isMember(GameGroup group, Long userId) {
        return group.getOwner().getId().equals(userId) || memberRepository.existsByGroupIdAndUserId(group.getId(), userId);
    }

    private GroupResponse toResponse(GameGroup group, User user) {
        return toResponse(group, user == null ? null : user.getId());
    }

    private GroupResponse toResponse(GameGroup group, Long userId) {
        return GroupResponse.from(group, memberRepository.countByGroupId(group.getId()),
                userId != null && isMember(group, userId));
    }
