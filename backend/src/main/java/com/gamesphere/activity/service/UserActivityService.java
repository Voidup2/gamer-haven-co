...
public class UserActivityService {
    ...
    public UserActivityService(UserActivityRepository repository, UserRepository userRepository) { this.repository=repository; this.userRepository=userRepository; }

    @Transactional
    public UserActivityResponse record(User user, UserActivity.ActivityType type, String title, String description, String referenceType, String referenceId) {
        return UserActivityResponse.from(repository.save(new UserActivity(user,type,title,description,referenceType,referenceId)));
    }

    @Transactional(readOnly = true)
    public Page<UserActivityResponse> mine(UserActivity.ActivityType type, Pageable pageable) {
        Long userId=currentUser().getId();
        Page<UserActivity> page=type == null ? repository.findByUserIdOrderByCreatedAtDesc(userId,pageable) : repository.findByUserIdAndActivityTypeOrderByCreatedAtDesc(userId,type,pageable);
