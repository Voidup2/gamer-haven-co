...
function Browse() {
  ...
  const sortConfig = {
    popular: { sortBy: "reviewCount", direction: "desc" as const },
    rating: { sortBy: "rating", direction: "desc" as const },
    recent: { sortBy: "releaseDate", direction: "desc" as const },
    alpha: { sortBy: "title", direction: "asc" as const },
  }[sort];
  ...
  const gamesQuery = useQuery({
    ...
    queryFn: () =>
      getGames({
        ...
        coop: coop || undefined,
        freeToPlay: freeOnly || undefined,
        size: 24,
        ...sortConfig,
      }),
  });

  const results = gamesQuery.data?.content ?? [];
    ...
    <div className="space-y-6 p-4 sm:p-6">
      <header className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          {gamesQuery.isLoading ? "Loading games..." : `${gamesQuery.data?.totalElements ?? 0} game${(gamesQuery.data?.totalElements ?? 0) === 1 ? "" : "s"} matching your filters`}
        </p>
      </header>
          ...
          <div className="glass space-y-5 rounded-2xl p-4">
            ...
            <fieldset className="space-y-3">
              ...
              <div className="flex items-center gap-2">
                <Checkbox id="mp" checked={multiplayer} onCheckedChange={(v) => setMultiplayer(v === true)} />
                <Label htmlFor="mp" className="font-normal">Multiplayer</Label>
              </div>
              <div className="flex items-center gap-2">
                <Checkbox id="coop" checked={coop} onCheckedChange={(v) => setCoop(v === true)} />
                <Label htmlFor="coop" className="font-normal">Co-op</Label>
              </div>
              <div className="flex items-center gap-2">
                <Checkbox id="free" checked={freeOnly} onCheckedChange={(v) => setFreeOnly(v === true)} />
                <Label htmlFor="free" className="font-normal">Free to play</Label>
              </div>
            </fieldset>
            <Button
              variant="outline"
        ...
        <div>
          {gamesQuery.isError ? (
            <div className="glass rounded-2xl p-10 text-center">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {gamesQuery.error instanceof Error ? gamesQuery.error.message : "Please make sure the backend is running on port 8080."}
              </p>
            </div>
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            {genres.slice(0, 6).map((g) => (
              <Badge key={g} variant="secondary" className="cursor-pointer rounded-lg" onClick={() => setGenre(g)}>
                {g}
              </Badge>
            ))}
          </div>
